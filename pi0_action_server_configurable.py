...
class ConfigurablePI0ActionServer(ActionServer):
    ...
    def _initialize_field_mappings(self):
        ...
        # Create component-based field mappings for RPC interface
        self.state_fields = {}
        self.action_fields = {}
        ...
        # Create state fields from slicing configuration for RPC
        if self.state_slicing:
            current_idx = 0
            for component_name, (start, end) in self.state_slicing.items():
                slice_len = end - start
                self.state_fields[component_name] = (current_idx, current_idx + slice_len)
                current_idx += slice_len
        else:
            # Fallback to simple mapping if no slicing
            self.state_fields = {"observation/state": (0, state_dim)}

        # Create action fields from slicing configuration for RPC
        if self.action_slicing:
            current_idx = 0
            for component_name, (start, end) in self.action_slicing.items():
                slice_len = end - start
                self.action_fields[component_name] = (current_idx, current_idx + slice_len)
                current_idx += slice_len
        else:
            # Fallback to simple mapping if no slicing
            self.action_fields = {"actions": (0, action_dim)}

        print(f"State dimensions: {state_dim}, fields: {self.state_fields}")
        print(f"Action dimensions: {action_dim}, fields: {self.action_fields}")
        print(f"State slicing: {self.state_slicing}")
        print(f"Action slicing: {self.action_slicing}")

    def _slice_array(self, array: np.ndarray, field_mappings: Dict[str, Tuple[int, int]]) -> np.ndarray:
        """Slice array based on field mappings."""
        if not field_mappings:
            return array
        
        # Extract slices from field mappings
        slices = []
        for field_name, (start, end) in field_mappings.items():
    ...
    def _unslice_array(
        self, 
        array: np.ndarray, 
        field_mappings: Dict[str, Tuple[int, int]], 
        target_dim: int
    ) -> np.ndarray:
        """Reconstruct array from field mappings."""
        if not field_mappings:
            return array
        ...
        # Fill in slices from field mappings
        current_idx = 0
        for field_name, (start, end) in field_mappings.items():
            slice_len = end - start
            if current_idx + slice_len <= len(array):
                result[start:end] = array[current_idx:current_idx + slice_len]
                current_idx += slice_len
    ...
    def process_request(self, request: GetPolicyActionRequest) -> Union[GetPolicyActionResponse, ServerError]:
        ...
        try:
            ...
            if request.states and request.states[-1]:  # Use latest state
                ...
                sliced_state = self._slice_array(state_vector, self.state_fields)
